...
#SBATCH --output=/cellar/users/aklie/data/datasets/igvf_sc-islet_10X-Multiome/bin/data_analysis/slurm_logs/pycistopic/%x.%A_%a.out
...
script=/cellar/users/aklie/data/datasets/igvf_sc-islet_10X-Multiome/bin/data_analysis/pycistopic/runModels_lda_mallet.py

# Configure input arguments that likely will not change
timecourses=(A2_3-cyt A2_control A2_dex A2_ext4 A2_IFNg H1_control H1_palmitate)
timecourse=${timecourses[$SLURM_ARRAY_TASK_ID-1]}
inputcisTopic_objs=($(printf "/cellar/users/aklie/data/datasets/igvf_sc-islet_10X-Multiome/analysis/2023_11_27/pycistopic/archr/%s/%s.pkl\n" "${timecourse}" "${timecourse}")) 
mallet_binary=/cellar/users/aklie/opt/Mallet-202108/bin/mallet
save_path=($(printf "/cellar/users/aklie/data/datasets/igvf_sc-islet_10X-Multiome/analysis/2023_11_27/pycistopic/archr/%s/mallet\n" "${timecourse}"))
