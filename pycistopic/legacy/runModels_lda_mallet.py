...
def make_argument_parser():
    ...
    parser.add_argument(
        "--output", "-o", type=str, required=True, help="Path to save final model list."
    )
    parser.add_argument(
        "--n_topics",
        "-nt",
    ...
    parser.add_argument(
        "--n_cpu", "-c", type=int, required=True, help="Number of cores"
    )
    parser.add_argument(
        "--n_iter",
        "-it",
    ...
    parser.add_argument(
        "--alpha", "-a", type=int, required=False, default=50, help="Alpha value"
    )
    parser.add_argument(
        "--alpha_by_topic",
        "-abt",
    ...
    parser.add_argument(
        "--eta", "-e", type=float, required=False, default=0.1, help="Eta value."
    )
    parser.add_argument(
        "--eta_by_topic",
        "-ebt",
...
def main():
    ...
    if not os.path.exists(os.path.dirname(output)):
        print("Creating directory:", os.path.dirname(output))
    ...
    if save_path == "None":
        ...
        if not os.path.exists(os.path.dirname(save_path)):
            print("Creating directory:", os.path.dirname(save_path))
    ...
    # Save
    with open(os.path.join(output, "models.pkl"), "wb") as f:
