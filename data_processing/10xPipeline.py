...
def generate_matrix(args):
	...
	with open(tmp_R, 'w') as tR:
		...
		print('''unique_rows <- unique(data$V1)''', file=tR)
		print('''unique_cols <- unique(data$V2)''', file=tR)
		print('''row_mapping <- match(data$V1, unique_rows)''', file=tR)
		print('''col_mapping <- match(data$V2, unique_cols)''', file=tR)
		print('''sparse.data <- sparseMatrix(i=row_mapping, j=col_mapping, x=data$V3)''', file=tR)
		print('''rownames(sparse.data) <- unique_rows''', file=tR)
		print('''colnames(sparse.data) <- unique_cols''', file=tR)
...
def process_args():
	...
	align_group.add_argument('-ref', '--reference', required=False, type=str, default='/cellar/users/aklie/data/ref/genomes/hg38/GCA_000001405.15_GRCh38_no_alt_analysis_set.fna', help='Path to the reference genome')
	...
	matrix_group.add_argument('--chrom-sizes', required=False, type=str, default='/cellar/users/aklie/data/ref/genomes/hg38/GRCh38_EBV.chrom.subset.sizes', help='Chromosome sizes file from UCSC')
	matrix_group.add_argument('--blacklist-file', required=False, type=str, default='/cellar/users/aklie/data/ref/blacklists/hg38/ENCFF356LFX.bed', help='BED file of blacklisted regions')
	matrix_group.add_argument('--promoter-file', required=False, type=str, default='/cellar/users/aklie/data/ref/genomes/promoters/gencode_v32.gene_info_promoters_2kb.bed', help='BED file of autosomal promoter regions')
